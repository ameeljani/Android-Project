...
public class PlayRecordedGame extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for(int i = 0; i < game.getMoves().size();i++) {
            ...
            System.out.println(game.getMoves().get(i).getOrigfile()+ " " + game.getMoves().get(i).getOrigrank()+" *** "+game.getMoves().get(i).getDestfile() + " " + game.getMoves().get(i).getDestrank());
    ...
    public void Cycle (View view){
        ...
        if(toexec.getisprom())
        {
            int bf=ChessPiece.rankToInt(toexec.getOrigfile());
            int br=ChessPiece.rankToInt(toexec.getOrigrank());
            Pawn p=(Pawn)board[bf][br];

            int df=ChessPiece.rankToInt(toexec.getDestfile());
            int dr=ChessPiece.rankToInt(toexec.getDestrank());

            Pawn.promote(board,df,dr,'Q', side);

            board[bf][br]=null;

            ImageButton orbut=getbutton(toexec.getOrigfile(), toexec.getOrigrank());
            ImageButton dbut=getbutton(toexec.getDestfile(), toexec.getDestrank());
